...
What happens to every frame:
  ...
  * the bottom edge fades to white, because the shadow touches the frame
    border in the second half of the clip;
...
FEATHER_FROM = 0.955  # start of the bottom fade (fraction of the height)
QUALITY = 78
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        t = np.clip((rows - FEATHER_FROM) / (1.0 - FEATHER_FROM), 0.0, 1.0)
        feather = t * t * (3.0 - 2.0 * t)  # smoothstep
        ...
        for index in range(FIRST_FRAME, LAST_FRAME + 1):
            ...
            frame = frame * (1.0 - feather) + 255.0 * feather
